...
const getPassengerById = async (id, cid) => {
  try {
    const result = await pool.query(
      "SELECT * FROM passengers WHERE id = $1 AND cid = $2",
      [id, cid]
    );
...
const getPassengerByThreadID = async (threadID) => {
  try {
    const result = await pool.query(
      "SELECT * FROM passengers WHERE threadID = $1",
      [threadID]
    );
    return result.rows[0];
  } catch (error) {
  ...
  try {
    ...
    if (passenger.cid !== cid) {
      ...
      return res
        .status(401)
        .json({
          message: "Unauthorized - You can only delete your own details",
        });
    }
...
const createPassenger = async (req, res) => {
  ...
  const { name, CID, gender, mobilenumber, threadID } = req.body;

  try {
    // Check if the passenger already exists
    const checkQuery = `SELECT * FROM passengers WHERE CID = $1`;
    const checkValues = [CID];
    const existingPassenger = await pool.query(checkQuery, checkValues);

    if (existingPassenger.rows.length > 0) {
      // Check if the passenger already exists
      const checkQuery = `UPDATE passengers
      SET threadID = $1
      WHERE CID = $2;
      `;
      const checkValues = [threadID, CID];
      const result = await pool.query(checkQuery, checkValues);
      // console.log(result.rows);
      // Passenger already exists
      return res.status(200).json({ error: "Passenger already exists." });
    }

    // Insert the new passenger
    const insertQuery = `
      INSERT INTO passengers (name, CID, gender, mobilenumber, threadID)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `;
    const insertValues = [name, CID, gender, mobilenumber, threadID];
...
const checkDuplicateCID = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT * FROM passengers WHERE cid = $1", [
      cid,
    ]);
